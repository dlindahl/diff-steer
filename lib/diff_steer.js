...
function diffSteer(leftRightAxis, upDownAxis, maxAxis, minAxis, maxSpeed, axisFlip) {
  ...
  leftMotorScale = (upDownAxis * axisFlip) * (1 + direction);
  leftMotorScale = clamp(leftMotorScale, minAxis, maxAxis); // Govern Axis to Minimum and Maximum range
  rightMotorScale = (upDownAxis * axisFlip) * (1 - direction);
  ...
  leftMotorOutput = leftMotorScale + leftMotorNoThrottleScale;
  leftMotorOutput = clamp(leftMotorOutput, minAxis, maxAxis);
  rightMotorOutput = rightMotorScale + rightMotorNoThrottleTurnScale;
  dblMotorB = clamp(rightMotorOutput, minAxis, maxAxis);
